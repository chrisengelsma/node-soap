...
WSSecurityCert.prototype.postProcess = function (xml, envelopeKey) {
  ...
  var xmlWithSec = insertStr(secHeader, xml, xml.indexOf('</s:Header>'));

  var references = this.signatureTransformations;

  this.signer.addReference("//*[name(.)='o:Security']/*[local-name(.)='Timestamp']", references);

  this.signer.computeSignature(xmlWithSec);

  return insertStr(this.signer.getSignatureXml(), xmlWithSec, xmlWithSec.indexOf('</o:Security>'));
};
